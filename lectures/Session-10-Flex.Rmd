...
```{r}
avg_price <- wine %>% filter(variety=="Pinot Noir") %>% summarize(price=mean(price))
gauge(avg_price$price, min = 0, max = 100, symbol = '%', 
...
```{r}
avg_points <- wine %>% filter(variety=="Pinot Noir") %>% summarize(points=mean(points))
gauge(avg_points$points, min = 0, max = 100, symbol = '%', 
      gaugeSectors(success = c(80, 100), warning = c(40, 79), danger = c(0, 39)))
```
...
```{r}
choices <- wine %>% filter(variety=="Pinot Noir") %>% count()
